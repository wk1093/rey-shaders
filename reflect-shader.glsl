...
void main() {
    	if (gl_FragCoord.y/u_resolution.y > .5)
    gl_FragColor = mainF(gl_FragCoord.xy);
    else
	gl_FragColor = mainF(vec2(gl_FragCoord.x+(2.*noise(gl_FragCoord.xy*u_time*random(gl_FragCoord.xy*u_time)))-1., (u_resolution.y/2.)+(u_resolution.y/2.-gl_FragCoord.y)+(2.*noise(gl_FragCoord.xy*u_time*random(gl_FragCoord.xy*u_time)))-1. ))*vec4(.1,.3,1.,1.);
}
